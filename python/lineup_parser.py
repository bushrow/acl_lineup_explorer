...
class LineupParser:
    ...
    def _segment_lineup_image(
        ...
        blank_cols = [
            ix for ix, col in enumerate(artists_im_arr.T) 
            if all(px > black_threshold for px in col)
        ]
        headliners_end_col = max(
            col for col in blank_cols 
        ...
        other_artists_im_arr = im_arr[header_end_row:footer_start_row, headliners_end_col:]
    ...
    def _extract_vertical_artist_names(
        ...
        blocks = [
            block for block in textract_result['Blocks'] 
    ...
    def parse_lineup(self, output_filenames=('headliners', 'other_artists')):
        ...
        headliner_names = self._extract_vertical_artist_names(image_filename=headliners_fn)
        other_artist_names = self._extract_horizontal_artist_names(image_filename=other_artists_fn)
    ...
    def parse_artist_names(
        ...
        for artist_name in headliner_names + other_artist_names:
            name, weekend_one, weekend_two = self._parse_weekends_from_artist_name(artist_name)
            artist_dict = {
                'artist': name,
